...
| **Class Hubs inside Notes** | `src/ui/mobileClassHubsUi.js`, `js/modules/notes-storage.js` | `mobile.js`, `src/reminders/reminderController.js`, `js/reminders.js`, `functions/api/assistant-chat.ts` | a separate class store, checklist store, class-only note editor, or second freeform AI input | Hubs are class-kind Notes folders. Notes use `folderId`; linked follow-ups and optional check-list cues remain canonical Reminders using `metadata.classHubId`. AI organisation is explicit and review-first: it reuses the universal input, sends only the new thought and class name, and writes only after confirmation. |
