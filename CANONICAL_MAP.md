...
| **Primary runtime** | `mobile.html`, `mobile.js` | `js/services/navigation-service-v2.js` | removed desktop `404.html` shell and its exclusive scripts; removed `js/storage.js`, `js/ui.js`, `js/entries.js`; legacy runtime dirs `legacy/*` and `memory/*` | Mobile runtime is the only product shell. The retained `404.html` file is a dependency-free redirect for invalid addresses, not an app runtime. |
...
| **Capture** | `src/core/capturePipeline.js` | public wrapper: `js/services/capture-service.js`; related callers: `mobile.js`, `src/services/intentRouter.js` | any new raw capture path | The real capture implementation lives in `src/core/capturePipeline.js`. Treat `js/services/capture-service.js` as the public wrapper, not the implementation owner. The old desktop capture path and `api/capture.js` endpoint have been removed. |
...
| **Entry / capture UI** | `mobile.html`, `mobile.js`, `src/reminders/reminderController.js` | note-to-reminder actions in `mobile.js` / `src/ui/mobileNotesShellUi.js` | retired mobile inbox/category UI and removed entries wrapper/UI modules | Universal capture is the sole visible freeform entry surface. Inbox remains a non-visible processing/storage layer. Do not recreate the removed entries UI path. |
...
| **Class Hubs inside Notes** | `src/ui/mobileClassHubsUi.js`, `js/modules/notes-storage.js` | `mobile.js`, `src/reminders/reminderController.js`, `js/reminders.js`, `functions/api/assistant-chat.ts` | a separate class store, checklist store, class-only note editor, or second freeform AI input | Hubs are class-kind Notes folders. Notes use `folderId`; linked follow-ups and optional check-list cues remain canonical Reminders using `metadata.classHubId`. A hub exposes one Add note path: the universal input appears while the hub stays open, AI review renders inside the hub, and nothing is written until confirmation. |
| **Memory Coach** | `js/services/recall-service.js`, `src/ui/mobileMemoryCoachUi.js`, `src/services/inboxService.js` | `mobile.js`, `mobile.html`, `css/capture.css`, `src/services/firestoreSyncService.js` | the retired passive `#memoryRecallList` rendering path; any separate practice store or second raw capture input | Memory Coach is a dedicated `coach` view for opt-in retrieval practice across names, facts, wording, steps, and personal details. Practice cards are hidden deferred-review Inbox entries with `metadata.type = "memory-card"` and `metadata.memoryCoach`. Word Help can supply expression cards, while a structured prompt-and-answer form creates general cards. Normal Inbox readers filter them; sync and backup retain the full Inbox. Notes and Reminders are not used. |
...
| **Assistant UI** | `mobile.html`, `mobile.js` | `js/services/assistant-service.js`, `src/components/*` | removed `js/assistant.js`, orphaned `src/ui/chatUI.js`, and root desktop `assistant.js` | The universal thinking bar and assistant thread are owned by `mobile.js`; there is no separate loaded assistant UI controller. |
...
| **Navigation** | `js/services/navigation-service-v2.js`, `mobile.html` | `mobile.js` and other `app:navigate` event dispatchers | removed `js/router.js`, `js/navigation.js`, and duplicate `js/services/navigation-service.js`; any new routing layer | Active mobile navigation has one controller. `404.html` redirects invalid addresses without implementing navigation. |
...
- **Opt-in recall practice** → hidden memory-card Inbox entries
