...
| **Primary runtime** | `mobile.html`, `mobile.js` | `js/services/navigation-service-v2.js` | legacy `404.html` shell and `js/router.js`; removed `js/storage.js`, `js/ui.js`, `js/entries.js`; legacy runtime dirs `legacy/*` and `memory/*` have been removed | Mobile runtime is the active product shell, but `mobile.js` is still overloaded and should be reduced over time. |
...
| **Navigation** | `js/services/navigation-service-v2.js`, `mobile.html` | `mobile.js` and other `app:navigate` event dispatchers | legacy `js/router.js` in `404.html`; removed `js/navigation.js` and duplicate `js/services/navigation-service.js`; any new routing layer | Active mobile navigation has one controller. Keep legacy-shell routing isolated until the `404.html` retirement pass. |
