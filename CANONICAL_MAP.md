...
| **Memory Coach** | `js/services/recall-service.js`, `src/ui/mobileMemoryCoachUi.js`, `src/services/inboxService.js` | `mobile.js`, `mobile.html`, `css/capture.css`, `src/services/firestoreSyncService.js` | the retired passive `#memoryRecallList` rendering path; any separate practice store or second capture input | Practice is opt-in. Vocabulary cards are hidden deferred-review Inbox entries with `metadata.type = "memory-card"` and `metadata.memoryCoach`. Normal Inbox readers filter them; sync and backup retain the full Inbox. Notes and Reminders are not used. |
