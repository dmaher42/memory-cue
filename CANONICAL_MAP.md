...
| **Class Hubs inside Notes** | `src/ui/mobileClassHubsUi.js`, `js/modules/notes-storage.js` | `mobile.js`, `src/reminders/reminderController.js`, `js/reminders.js` | a separate class store, checklist store, or class-only note editor | Hubs are class-kind Notes folders. Notes use `folderId`; linked follow-ups and optional check-list cues remain canonical Reminders using `metadata.classHubId`. |
