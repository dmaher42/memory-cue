...
| **Primary runtime** | `mobile.html`, `mobile.js` | `js/navigation.js`, `js/router.js` | removed `js/storage.js`, `js/ui.js`, `js/entries.js`; legacy runtime dirs `legacy/*` and `memory/*` have been removed | Mobile runtime is the active product shell, but `mobile.js` is still overloaded and should be reduced over time. |
...
| **Assistant UI** | `mobile.html`, `mobile.js`, `js/assistant.js` | `js/services/assistant-service.js`, `src/components/*` | removed orphaned `src/ui/chatUI.js`; root `assistant.js` in old runtime contexts | Assistant UI is still split between the mobile shell and `js/assistant.js` and should be changed carefully. |
...
| **Navigation** | `js/navigation.js`, `js/router.js`, `mobile.html`, `mobile.js` | local event-driven view toggles, hash-route overlap | removed navigation code from `js/entries.js`; any new routing layer | Navigation still overlaps across multiple mechanisms. Do not add another one. |
