...
|---|---|---|---|---|---|
| Universal Capture | One visible AI-assisted text or voice input for reminders, notes, and questions. | Fixed `#thinkingBarContainer` / `#thinkingBarInput` plus `#thinkingBarVoiceButton` in `mobile.html`; wired through `mobile.js` into the canonical capture pipeline. | Reminder, note, or Inbox storage selected by the canonical capture decision. | Keep as the single visible capture bar | Voice fills the same input for review before Send. The duplicate reminder-only form and its UI wiring have been removed. `memoryCueQuickAddNow({ forceText })` remains as a non-visual compatibility and regression seam. |
...
| Class Hubs | Keep each class's Notes, follow-up list, optional check-list cue, and review-first AI organisation together. | `Class hubs` in Notes > Saved notes, owned by `src/ui/mobileClassHubsUi.js`; one `Add note` action reveals the universal input without leaving the hub and returns the review inline. | Existing `memoryCueFolders`, `memoryCueNotes`, and Reminder records linked by stable hub id; AI drafts remain runtime-only. | Keep inside Notes | No new store, parallel note-creation route, or second freeform input. Saved note rows open the regular editor for later editing. The organiser sends only the new thought and class name, saves nothing before confirmation, and never invents a date. Confirmed Notes become retrievable through existing search/embedding paths rather than training a model. |
...
| Memory Coach | Personal retrieval practice for names, facts, wording, steps, lists, and other meaningful details, with clues, reveal, honest self-rating, and adaptive return intervals. | Dedicated `Coach` destination in bottom navigation; UI controller in `src/ui/mobileMemoryCoachUi.js`; structured forms create single-answer memories or lists with optional ordering, while Word Help can still supply expression cards. | Opt-in hidden memory-card entries inside `memoryCueInbox`; list items and ordering stay within the same card metadata, with no new storage key or AI call during review. | Keep and evolve | General, list, and expression cards use the same recall engine and spacing history. Normal Inbox readers filter practice cards, while sync and backups retain them. Existing Notes and Reminders remain byte-for-byte outside this flow. |
