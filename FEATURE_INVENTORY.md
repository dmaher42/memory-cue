...
| Class Hubs | Keep each class's Notes, follow-up list, and optional check-list cue together. | `Class hubs` in Notes > Saved notes, owned by `src/ui/mobileClassHubsUi.js`. | Existing `memoryCueFolders`, `memoryCueNotes`, and Reminder records linked by stable hub id. | Keep inside Notes | No new store or parallel note editor. Detailed list items suppress notifications; an explicitly dated generic cue uses the normal Reminder scheduler. |
