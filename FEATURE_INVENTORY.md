...
| Memory Coach | Contextual expression and vocabulary retrieval practice with clues, reveal, honest self-rating, and adaptive return intervals. | `Memory coach` launcher in the Capture header; UI controller in `src/ui/mobileMemoryCoachUi.js`; Word Help offers urgent wording or meaning-first coaching and supplies explicit `Practise` actions. | Opt-in hidden memory-card entries inside `memoryCueInbox`; no new storage key and no AI call during review. | Keep and evolve | Expression cards retain the user's original rough wording as the retrieval situation. Normal Inbox readers filter practice cards, while sync and backups retain them. Existing Notes and Reminders remain byte-for-byte outside this flow. |
