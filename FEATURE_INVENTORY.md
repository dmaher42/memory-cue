...
| Memory Coach | Personal retrieval practice for names, facts, wording, steps, and other meaningful details, with clues, reveal, honest self-rating, and adaptive return intervals. | Dedicated `Coach` destination in bottom navigation; UI controller in `src/ui/mobileMemoryCoachUi.js`; a structured prompt-and-answer form creates general memories, while Word Help can still supply expression cards. | Opt-in hidden memory-card entries inside `memoryCueInbox`; no new storage key and no AI call during review. | Keep and evolve | General and expression cards use the same recall engine and spacing history. Normal Inbox readers filter practice cards, while sync and backups retain them. Existing Notes and Reminders remain byte-for-byte outside this flow. |
