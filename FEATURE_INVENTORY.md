...
| Class Hubs | Keep each class's Notes, follow-up list, optional check-list cue, and review-first AI organisation together. | `Class hubs` in Notes > Saved notes, owned by `src/ui/mobileClassHubsUi.js`; `Organise a thought` temporarily reuses the universal Capture input. | Existing `memoryCueFolders`, `memoryCueNotes`, and Reminder records linked by stable hub id; AI drafts remain runtime-only. | Keep inside Notes | No new store, parallel note editor, or second freeform input. The organiser sends only the new thought and class name, saves nothing before confirmation, and never invents a date. Confirmed Notes become retrievable through existing search/embedding paths rather than training a model. |
