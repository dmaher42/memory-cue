...
| `memoryCueFolders` | Array of folder objects `{ id,name,order,kind? }`; Class Hubs use `kind: "class-hub"` | `js/modules/notes-storage.js`, `js/reminders.js` (reflection folder helper) | `js/modules/notes-storage.js`, `mobile.js`, `js/reminders.js` |
| `memoryCue:offlineReminders` | Array of reminder/task objects (offline source of truth for reminders UI); Class Hub links use `metadata.classHubId` | `js/reminders.js` (`persistOfflineReminders`) | `js/reminders.js` (`loadOfflineRemindersFromStorage`) |
