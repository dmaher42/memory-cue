...
6. `src/ui/mobileMemoryCoachUi.js` runs contextual retrieval, clue, reveal, and self-rating in the dedicated Coach view. General cards can hold one answer or a short list; list cards retain their individual items and whether order matters inside the same hidden Inbox metadata. `js/services/recall-service.js` calculates due items and the next interval without another AI call.
...
12. Saved vocabulary/expression cards offer Practise wording from the library and review summary. Four local audience prompts ask the user to formulate a sentence before comparing it with the saved expression, meaning, example and alternatives. This is self-assessment, not AI grading. The confirmed result uses the same Inbox card and bounded history (`isApplication`, `applicationContext`, `applicationCount`), preserving its recall schedule, mastery and pause state. Cancelling writes nothing; stale card snapshots cannot receive a result. These exercises make no AI request and collect no new freeform input.
