...
## Big picture
- App Type: Progressive Web App (PWA) implemented as a static site with a service worker (`service-worker-v3.js`, registered by `js/register-service-worker-v2.js`). Key front-end code lives under `js/`, `modules/`, and top-level entry files like `app.js`, `mobile.js`, and `index.html`.
...
- `service-worker-v3.js` — offline & reminders logic (the live service worker, registered via `js/register-service-worker-v2.js`). Note constants like `CACHE_VERSION` (bump when changing precache), `REMINDER_STORE_NAME`, and message types used to update scheduled reminders.
...
- Service worker cache/versioning: When changing precached shell files or asset paths, increment `CACHE_VERSION` in `service-worker-v3.js` to force clients to update.
- Reminders persistence: The service worker (`service-worker-v3.js`) uses IndexedDB store `memory-cue-reminders` / object store `scheduled`. Messages to SW use `type: 'memoryCue:updateScheduledReminders'` and `'memoryCue:checkScheduledReminders'` — prefer those shapes when driving the SW from pages.
...
- Updating precache list: edit `SHELL_URLS` in `service-worker-v3.js` and bump `CACHE_VERSION` so clients fetch the new assets.
